import { mix, rgba, parseToRgb } from 'polished';
import { calculateAdvanced } from '../math';

export const fn = (handle) => (...args) => (t) => {
  const input = args.map((arg) => {
    const value = typeof arg === 'function' ? arg(t) : arg;
...
const luminance = (color) => {
  ...
  Object.entries(rgb).forEach(([name, value]) => {
    rgb[name] =
      value / 255 < 0.03928 ? value / 255 / 12.92 : luminanceList[value];
  });
...
const opaque = (background, foreground) => {
  const opacity = parseToRgb(foreground).alpha || 1;
  return mix(opacity, rgba(foreground, 1), background);
};
...
export const tintColor = fn(([weight, color], t) => {
  return mix(weight, t.white, color);
});

// Shade a color: mix a color with black
export const shadeColor = fn(([weight, color], t) => {
  return mix(weight, t.black, color);
});

// Shade the color if the weight is positive, else tint it
export const shiftColor = fn(([weight, color], t) => {
  const handle =
    weight > 0 ? shadeColor(weight, color) : tintColor(-weight, color);
